...
tab1 <- data.table::data.table(origin = ids, destination = dest,
...
year <- 2015
tab2 <- data.table::data.table(ags_old = ids, ags_new = ids_new, conv_p = conv, year = year)

#### do corrections for origin work?
tab_c <- MigStat:::correct_flow(tab1, tab2, ags_col = "origin")
data.table::setcolorder(tab_c, c("origin", "destination", "group", "flow"))
...
tab_c <- MigStat:::correct_flow(tab1, tab2, ags_col = "destination")
data.table::setcolorder(tab_c, c("origin", "destination", "group", "flow"))
tab_c <- tab_c[order(origin, destination, group), ]
